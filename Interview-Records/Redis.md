...
### 缓存击穿怎么办？
- 缓存击穿 即 缓存未命中 + 该数据在短时间受到大量请求，导致所有请求被直接打到数据库，造成数据库压力骤增甚至崩溃
- 解决方法：
    - 热点数据提前预热
    - 设置互斥锁(分布式锁)
        - 当缓存未命中时，让第一个请求加载数据库数据并更新缓存，其他请求等待第一个请求结束后直接使用缓存数据。
        - 可以使用分布式锁（如 Redis 或 Zookeeper）来实现锁机制，从而限制同一时间只有一个请求去查询数据库。
    - 设置缓存重建机制
        - 为每一个热点缓存数据设置提前失效时间，即在缓存到期前主动去更新缓存。
    - 使用布隆加速器拦截无效请求
        - 快速判断某个数据是否存在。它能够过滤掉一部分数据库中不存在的数据，从而避免将所有的无效请求都打到数据库。

### 布隆过滤器怎么实现的？
- 概率性数据结构，有一定概率误判
- 实现步骤：
    - 初始化比特数组: 创建一个大小为m的比特数组 bit_array, 初始化全为0
    - 选择多个hash函数，每个hash函数将输入元素映射到 0 到 m-1 范围内的某个位置上（bit_array），更新bit_array对应位置的元素
    - 检查时，使用同一组hash function来计算得到k个位置，如果k个位置都为1，则该元素可能存在，继续处理；如果任意位置是0，则可以确定不存在
