...
void sys__exit(int exitcode) {
    // print the exit code
    kprintf("Process exit with exitCode: %d\n", exitcode);

    // perform any necessary cleanup and terminate the thread
    thread_exit();
}

/*
 * added syscall printint()
 * if the integer is divisible by 2, return 0
*/
int sys_printint(int code) {
	// print
	kprintf("\n\nPrintInt: %d\n", code);
	if (code % 2 == 0) {
		kprintf("code is divisible by 2, return : %d\n", 0);
		return 0;
	} else {
		kprintf("code is not divisible by 2, return : %d\n", 1);
		return 1;
	}
}
